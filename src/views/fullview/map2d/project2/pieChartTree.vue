...
export default {
    data() {
        return {
            optionData: [
                { name: '转向架', value: 34, itemStyle: { color: '#91D9FE', opacity: 0.65 } },
                { name: '车体拆解', value: 33, itemStyle: { color: '#54E1AE', opacity: 0.65 } },
                { name: '车体拆解', value: 33, itemStyle: { color: '#496FF4', opacity: 0.65 } }
            ]
        }
    },
    watch: {
    ...
    methods: {
        initChart() {
            //构建3d饼状图
            let myChart = echarts.init(document.getElementById('myChart4'),null, {renderer: 'svg'})
        ...
        getPie3D(pieData, internalDiameterRatio) {
            ...
            //(第二个参数可以设置你这个环形的高低程度)
            let boxHeight = this.getHeight3D(series, 12) //通过传参设定3d饼/环的高度
                ...
                tooltip: {
                    ...
                    formatter: params => {
                        if (params.seriesName !== 'mouseoutSeries' && params.seriesName !== 'pie2d') {
                            let bfb = ((option.series[params.seriesIndex].pieData.endRatio - option.series[params.seriesIndex].pieData.startRatio) * 100).toFixed(0)
                            return (
